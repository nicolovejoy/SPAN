...
- **Dashboard UX backlog** — web app: ~~time range in PST~~ (done 2026-06-19: chart axis renders Pacific via per-point DST offset in `PowerChart.tsx`), a time-nav control beyond swiping, all-axes labels (2026-06-14 feedback); polling cadence (#5), relax CF Access login (#6), 1m smoothing (#7). Custom PWA icon. Optional: de-group HVAC into per-circuit lines in the explorer (low priority).
